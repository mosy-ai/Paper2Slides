...
from .paths import get_rag_checkpoint, get_summary_checkpoint
from .stages import run_generate_stage, run_plan_stage, run_rag_stage, run_summary_stage
from .state import STAGES, create_state, load_state, save_state
        ...
        for content_dir in sorted(project_dir.iterdir()):
            ...
            for mode_name in ["fast", "normal"]:
                ...
                if mode_dir.exists() and mode_dir.is_dir():
                    ...
                    has_summary = get_summary_checkpoint(
                        content_dir, dummy_config
                    ).exists()
                    ...
                    for config_dir in sorted(mode_dir.iterdir()):
                        if (
                            config_dir.is_dir()
                            and not config_dir.name.startswith(".")
                            and config_dir.name
                            not in [
                                "checkpoint_rag.json",
                ...
                for mode_name, has_rag, has_summary, configs in mode_dirs:
                    ...
                    if configs:
                        for name, state in configs:
                            ...
                            logger.info(
                                f"    {name}/ plan[{plan_icon}] generate[{gen_icon}]"
                            )
