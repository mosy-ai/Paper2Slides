...
def _replace_images_with_base64(
    markdown_content: str, markdown_base_path: str
) -> Tuple[List, int]:
    """
    ...
    for match in re.finditer(pattern, markdown_content, re.IGNORECASE | re.DOTALL):
        ...
        if Path(image_path).exists():
            ...
            if base64_str:
                ...
                image_count += 1
                logger.debug(
                    f"Embedded image at position {match.start()}: {image_path}"
                )
            else:
...
async def run_rag_stage(base_dir: Path, config: Dict) -> Dict:
    ...
    # ========== NORMAL MODE: Full RAG pipeline ==========
        ...
        from paper2slides.rag import RAG_PAPER_QUERIES, RAG_QUERY_MODES, RAGClient
        ...
        rag_config = RAGConfig.with_paths(
            storage_dir=str(storage_dir), output_dir=str(output_dir)
        )
        ...
        async with RAGClient(config=rag_config) as rag:
            ...
            logger.info(
                f"  Indexing completed: {batch_result.get('successful_rag_files', 0)} successful, {batch_result.get('failed_rag_files', 0)} failed"
            )
