    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": STYLE_PROCESS_PROMPT.format(user_style=user_style),
                }
            ],
...
class ImageGenerator:
    ...
    def _generate_poster(
        ...
        return [
            GeneratedImage(
                section_id="poster", image_data=image_data, mime_type=mime_type
            )
        ]
    ...
    def _format_single_section_markdown(
        self, section: Section, plan: ContentPlan
    ) -> str:
    ...
    def _filter_images(
        self, sections: List[Section], figure_images: List[dict]
    ) -> List[dict]:
