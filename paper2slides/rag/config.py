...
class ParserConfig:
    ...
    display_content_stats: bool = field(
        default_factory=lambda: os.getenv("DISPLAY_CONTENT_STATS", "true").lower()
        == "true"
    )
    """Whether to display content statistics during parsing."""

    enable_image_processing: bool = field(
        default_factory=lambda: os.getenv("ENABLE_IMAGE_PROCESSING", "true").lower()
        == "true"
    )
    enable_table_processing: bool = field(
        default_factory=lambda: os.getenv("ENABLE_TABLE_PROCESSING", "true").lower()
        == "true"
    )
    enable_equation_processing: bool = field(
        default_factory=lambda: os.getenv("ENABLE_EQUATION_PROCESSING", "true").lower()
        == "true"
    )
...
class BatchConfig:
    ...
    recursive_folder_processing: bool = field(
        default_factory=lambda: os.getenv("RECURSIVE_FOLDER_PROCESSING", "true").lower()
        == "true"
    )
...
class ContextConfig:
    ...
    context_mode: str = field(default_factory=lambda: os.getenv("CONTEXT_MODE", "page"))
